...
public class DoacaoDAO extends EntityDAO{
    ...
    public Doacao convertResultSet(ResultSet rst) throws PersistableException {
            Doacao ret = null;
        try {
            ret = new Doacao(rst.getBoolean("Recibo"),
                                  rst.getDate("Data"),
                                  rst.getString("Descricao"),
                                  rst.getString("Evento"),
                                  rst.getFloat("Valor"),
                                  rst.getInt("Tipo"),
                                  rst.getString("Destino"),
                                  rst.getInt("Projecto_Id"),
                                  rst.getInt("Doador_Id"));
    ...
    public Map<Integer,Float> getDoacoesMateriais(int Doacao) throws PersistableException {
        ...
        String sql = "select * from " + FacadeDAO.DOACAOMATERIALTABLE + " where Doacao_Id=?";
        PreparedStatement statement = null;
        ResultSet rst = null;
        try {
            statement = conn.prepareStatement(sql);           
            statement.setInt(1, Doacao);
            rst = statement.executeQuery();
            
            while(rst.next())
            {
                int Material_Id = rst.getInt("Material_Id");
                Float Quantidade = rst.getFloat("Quantidade");
                ret.put(Material_Id, Quantidade);
            }
